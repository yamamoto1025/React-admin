...
export const getReply = createAsyncThunk('replyApp/reply/getReply', async (replyId, { dispatch, getState }) => {
  try {
    ...
    if (reply) {
      ...
      if (reply.response && reply.response.length && reply.response.length > 0) {
...
export const updateReply = createAsyncThunk('replyApp/reply/updateReply', async (params, { dispatch, getState }) => {
  try {
    const { token } = await firebase.auth().currentUser.getIdTokenResult();
    if (!token) return null;
    const { id: orgId } = getState().auth.organization.organization;
...
export const removeReply = createAsyncThunk('replyApp/reply/removeReply', async (replyId, { dispatch, getState }) => {
  try {
    const { token } = await firebase.auth().currentUser.getIdTokenResult();
    if (!token) return null;
    const { id: orgId } = getState().auth.organization.organization;
