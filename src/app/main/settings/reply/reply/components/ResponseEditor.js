...
function ResponseEditor(props) {
  ...
  const currentResponses = useSelector(({ replyApp }) => replyApp.response.current);

  useEffect(() => {
    if (responses && responses[responseIndex]) {
      setTabPosition(tabMenu.find((menu) => menu.type === responses[responseIndex].type).key);
    }
  ...
  const handleSelectChange = (event) => {
    ...
    if (responses[responseIndex] && responses[responseIndex].type && responses[responseIndex].type === type) {
      dispatch(editResponse({ index: responseIndex, response: { ...responses[responseIndex] } }));
    } else {
      dispatch(editResponse({ index: responseIndex, response: { ...responses[responseIndex], type, data: null } }));
    }
          ...
          <div className="flex flex-row">
            <div className="flex flex-col sm:flex-row">
              <IconButton
                ...
                // }}
                onClick={() => {
                  dispatch(changeResponseOrder({ index: responseIndex, change: -1 }));
                }}
                size="large"
              >
                <KeyboardArrowUpIcon />
