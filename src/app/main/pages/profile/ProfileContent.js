...
function ProfileContent() {
  ...
  const organizations = useSelector(({ profilePage }) => profilePage.profile.organizations);
  ...
  function linkEmail(data) {
    ...
    const credential = firebaseService.Providers.email.credential(data.email, data.password);
    currentUser
  ...
  const EmailDialog = () => {
    return (
      <Dialog open={emailDialogOpen} onClose={() => setEmailDialogOpen(false)} maxWidth="sm">
        ...
        <form noValidate onSubmit={handleSubmit(linkEmail)} className="flex flex-col md:overflow-hidden">
    ...
    <motion.div variants={container} initial="hidden" animate="show">
      <div className="md:flex mx-32">
        <div className="flex flex-col flex-1">
          <Card component={motion.div} variants={item} className="w-full mb-32 rounded-16 shadow md:mt-0 mt-48">
            <AppBar position="static" elevation={0}>
                ...
                <div className="grid grid-flow-row-dense md:grid-cols-4 grid-cols-1">
                  ...
                  {profile.gender ? (
                    <div className="mb-24">
                      ...
                      <Typography>
                        {profile.gender.slice(0, 1).toUpperCase() + profile.gender.slice(1, profile.gender.length)}
                      </Typography>
                    </div>
                  ) : (
                    <></>
                  ...
                  {profile.team ? (
                    <div className="mb-24">
                      <Typography className="font-semibold mb-4 text-15">Team</Typography>
                  ...
                  {profile.createdAt ? (
                    <div className="mb-24">
                      ...
                      <Typography>{format(new Date(profile.createdAt), 'PP')}</Typography>
                    </div>
                  ) : (
                    <></>
                  )}
                  {profile.updatedAt ? (
                    <div className="mb-24">
                      <Typography className="font-semibold mb-4 text-15">Updated Date</Typography>
                      <Typography>{format(new Date(profile.updatedAt), 'PP')}</Typography>
                    </div>
                  ) : (
                    <></>
                  )}
                      ...
                      <TableBody>
                        {organizations.map((row) => (
                          <TableRow key={row.organization.name}>
                            ...
                            <TableCell>{row.organization.description}</TableCell>
