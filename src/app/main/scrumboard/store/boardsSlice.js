...
export const getBoards = createAsyncThunk('scrumboardApp/boards/getBoards', async (params, { dispatch, getState }) => {
...
export const newBoard = createAsyncThunk('scrumboardApp/boards/newBoard', async (board, { dispatch, getState }) => {
  const { token } = await firebase.auth().currentUser.getIdTokenResult();
  if (!token) return null;
  const { id: orgId } = getState().auth.organization.organization;
  ...
  dispatch(
    ...
    history.push({
      pathname: `/apps/boarddetail/${data.id}/${data.uri}`,
    });
  });
