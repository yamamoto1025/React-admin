...
function CategoryHeader(props) {
  ...
  function handleRemoveCategory() {
    ...
    dispatch(removeCategory()).then(() => {
      history.push('/woocommerce/categorys');
    });
  }
    ...
    <div className="flex flex-1 w-full items-center justify-between">
      <div className="flex flex-col items-start max-w-full min-w-0">
        ...
        <div className="flex items-center max-w-full">
          ...
          >
            {image ? (
              <img className="w-32 sm:w-48 rounded" src={image.src} alt={image.alt} />
            ) : (
              <img
          ...
          <div className="flex flex-col min-w-0 mx-8 sm:mc-16">
            <motion.div initial={{ x: -20 }} animate={{ x: 0, transition: { delay: 0.3 } }}>
              <Typography className="text-16 sm:text-20 truncate font-semibold">{name || 'New Category'}</Typography>
