...
function OrganizationListItem(props) {
  ...
  const redirectUrl = state && state.redirectUrl && state.redirectUrl !== '/organization' ? state.redirectUrl : '/';
    ...
    <motion.div initial={{ y: 20, opacity: 0 }} animate={{ y: 0, opacity: 1, transition: { delay: 0.1 } }}>
      <Card className={clsx('cursor-pointer', props.className)}>
        ...
        <CardActions disableSpacing>
          <Button size="small" color="primary" className="w-full h-48 text-xl" onClick={onSelect}>
            Select
          </Button>
        </CardActions>
      </Card>
